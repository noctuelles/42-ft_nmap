...
/*   Updated: 2024/09/12 13:50:49 by plouvel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
uint16_t
compute_tcphdr_checksum(in_addr_t source_ip, in_addr_t dest_ip, struct tcphdr tcphdr, uint8_t *tcp_data, size_t tcp_data_len) {
 ...
 * @param source_ip Source IP address in network byte order.
 * @param dest_ip Destination IP address in network byte order.
 * @param tcphdr The UDP header to compute the checksum from.
 * @param tcp_data The UDP data to compute the checksum from.
 * @param tcp_data_len The length of the UDP data in bytes.
 * @return uint16_t The computed checksum.
 */
uint16_t
compute_udphdr_checksum(in_addr_t source_ip, in_addr_t dest_ip, struct udphdr udphdr, uint8_t *udp_data, size_t udp_data_len) {
    uint8_t      buffer[IP_MAXPACKET];
